    ...
    if request.method == 'POST':
        ...
        for i in retrieved_participant:
            ...
            for j, award in enumerate(imgs):
                ...
                contest_font_part2 = ImageFont.truetype(settings.BASE_DIR) + '/CertificationModel/STZHONGS.TTF', 50)
                ...
                award_font = ImageFont.truetype(settings.BASE_DIR) + '/CertificationModel/STZHONGS.TTF', 100)
                draw.text(award_position, award_text, font=award_font, fill='white')

                sponsor_position = (1600 - 18 * (len(sponsor_text)), 1100)
                sponsor_font = ImageFont.truetype(settings.BASE_DIR) + '/CertificationModel/STZHONGS.TTF', 36)
                draw.text(sponsor_position, sponsor_text, font=sponsor_font, fill='white')

                time_position = (1600 - 18 * (len(time_text) - 3), 1150)
                time_font = ImageFont.truetype(settings.BASE_DIR) + '/CertificationModel/STZHONGS.TTF', 36)
                draw.text(time_position, time_text, font=time_font, fill='white')

                qrcode_position = (1005 - 18 * (len(qrcode_text)), 950)
                qrcode_font = ImageFont.truetype(settings.BASE_DIR) + '/CertificationModel/STZHONGS.TTF', 36)
